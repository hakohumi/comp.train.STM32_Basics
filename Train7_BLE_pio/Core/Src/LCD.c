...
void LCD_ClearBuffer(void) {
    LCD_WriteToBuffer(0, STR_LINE_BLANK, LCD_LINE_LEN);
    LCD_WriteToBuffer(8, STR_LINE_BLANK, LCD_LINE_LEN);
}

// 　好きな位置を選択して、書き換える
void LCD_WriteToBuffer(uint8_t i_WriteStartPos, uint8_t *i_str, uint8_t i_strLen) {
    uint8_t i, c;
...
void LCD_WriteToBufferInt(uint8_t i_WriteStartPos, uint16_t i_val, uint8_t i_Len) {
    int8_t i, c;
...
static int LCD_WriteNData(uint8_t *i_data, uint8_t i_len) {
    ...
    // 書き込み
    status = HAL_I2C_Master_Transmit(hi2c, LCD_ADDR << 1, l_buf, i_len + 1, 100);
    HAL_Delay(1);
    return status == HAL_OK;
}
