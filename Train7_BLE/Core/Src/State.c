...
// 初期化
void State_Init(uint8_t i_state) { SystemState = i_state; }
...
void State_runRealtimeBLEInput(void) {
    ...
    // 解決策1の流れ：①
    // エンドラインフラグが立ったとき、バッファの後ろから、探したい文字列の先頭文字をstrchrで検索し、ポインタの位置を取得
    // 　　　　　　　　②
    ...
    case LINBLE_STATE_COMMAND:
        ...
        if(LINBLE_GetEndLineFlg() == true) {
            ...
            if(l_strLength > 0) {
                ...
                if(Mystring_FindStrFromEnd((uint8_t *)&l_strBuf, 64,
                                           (uint8_t *)"ACKN\r\n", 6) == 1) {
                ...
                PrintUART((uint8_t *)"error linble run realtime ble input "
                                     "state command \r\n");
            }
    ...
    case LINBLE_STATE_ADVERTISE:
        ...
        if(LINBLE_GetEndLineFlg() == true) {
            ...
            if(l_strLength > 0) {
                ...
                } else if(Mystring_FindStrFromEnd((uint8_t *)&l_strBuf, 64,
                                                  (uint8_t *)"ACKN\r\n",
                                                  6) == 1) {
    ...
    case LINBLE_STATE_ONLINE:
        if(LINBLE_GetEndLineFlg() == false) {
            ...
            if(l_unreadCount > 0) {
                ...
                l_strLength = LINBLE_GetReceiveDataLast((uint8_t *)&l_strBuf,
                                                        l_unreadCount + 1);
                if(l_strLength > 0) {
...
void State_runRealtimeBLECentralInput(void) {
    ...
    case LINBLE_STATE_COMMAND:
        ...
        // リザルトメッセージ待機フラグが立っていたときのみ実行
        if(LINBLE_GetEndLineFlg() == true) {
            // if ((LINBLE_GetReceiveResultMesgWaitFlg() == true) &&
            // LINBLE_GetEndLineFlg()) == true) {
            l_strLength = LINBLE_GetReceiveData((uint8_t *)&l_strBuf, 64);

            // 何も入力されていない場合、エラー
            if(l_strLength <= 0) {
                PrintUART((uint8_t *)"error linble run realtime ble central "
                                     "input state command \r\n");
            } else {
                // ここから、コマンド実行フラグを元にして、
                // それぞれの処理をしたい
                ...
                } else if(LINBLE_GetCmdFlg(LINBLE_FLG_CMD_BTC) ==
                          true) { // BTCコマンドを実行した時の処理
...
uint8_t State_GetState(void) { return SystemState; }

void State_SetUpdateFlg(void) { State_UpdateFlg = true; }
