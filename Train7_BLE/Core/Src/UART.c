...
int8_t PrintUART(uint8_t *i_str) {
	...
	int status = HAL_UART_Transmit(this_huart, i_str,
			(uint16_t) (strlen((const char*) i_str)), 0xffff);
...
int8_t PrintUARTn(uint8_t *i_str, uint8_t i_size) {
	...
	int status = HAL_UART_Transmit(this_huart, &l_strBuf,
...
uint8_t PrintChar(uint8_t i_char) {
	...
	if (MyString_CheckCharCtrlCode(i_char) == true) {
		...
		status = HAL_UART_Transmit(this_huart, (uint8_t*) "  ", (uint16_t) 2,
				0xffff);
	}
	return status == HAL_OK;
}
...
int8_t PrintUARTInt(uint32_t i_var) {
	...
	int status = HAL_UART_Transmit(this_huart, l_buffer,
...
bool dprintUART(uint8_t *i_str, uint32_t i_var) {
	...
	int status = HAL_UART_Transmit(this_huart, l_buffer2,
